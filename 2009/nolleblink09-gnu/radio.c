...
interrupt (TIMERB0_VECTOR) timerb0_isr (void)
{
  timesincesent++;
...
{
  unsigned char w;
  if((((unsigned int)readdata[1] << 8 | readdata[2]) == myid)
     || ((unsigned int)readdata[1] << 8 | readdata[2]) == 0
     && ((unsigned int)readdata[5] << 8 | readdata[6]) != lastpkg)
    {
