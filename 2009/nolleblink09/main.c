...
void readpack(){
	...
	if(readdata[7] == 1){ // retrn a pong
	...
	else if(readdata[7] == 2){ // got a textmessage OBS IMPLEMENT MULTIPLE TIMES SCROLLING...
		...
		txpack((unsigned int)readdata[3]<<8|readdata[4], myadr,readdata[5]<<8|readdata[6],0,&readdata,0);
		
		sending = 0;
	}
	else if(readdata[7] == 3){ // show id nbr
		setstate(3);
	}
	else if(readdata[7] == 4){ // sync
		sync();
		txpack((unsigned int)readdata[3]<<8|readdata[4], myadr,readdata[5]<<8|readdata[6],0,&readdata,0);
//		setreceive();
	}
	else if(readdata[7] == 5){ // pong
	}
	else if(readdata[7] == 6){
	}
	else if(readdata[7] == 7){ // write text to flash
		writeflash(0);
		readflash();
		txpack((unsigned int)readdata[3]<<8|readdata[4], myadr,readdata[5]<<8|readdata[6],0,&readdata,0);
	}
	else if(readdata[7] == 8){ // writes text to flash and writes id from readdata[9]
		writeflash(1);
		readflash();
		txpack((unsigned int)readdata[3]<<8|readdata[4], myadr,readdata[5]<<8|readdata[6],0,&readdata,0);
		...
		if((TAR % 100) <= readdata[9] || readdata[11] == 33){ //got virus
...
unsigned char readflash(){
	...
	if(Flash_ptr[0] == 0x33 && Flash_ptr[1] == 0x44 && Flash_ptr[2] == 0x55 && Flash_ptr[3] == 0x66){ // valid data in flash
		myadr = Flash_ptr[4];
		setradioid(myadr);
		length = Flash_ptr[5];
